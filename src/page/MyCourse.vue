<template>
    <div class="body index" id="Course_Selection_web">
        <div class="Path" style="margin-top:20px">
            <i-select class="mr10" style="width:600px;" @on-change="getOptCourseList()" v-model="addCourse.ActivityID"> 
                    ...
                    <Tab-Pane v-for="(course,index) in courseList" :key="course.SerID" :name="course.SerID" :label="label(course.Name)">
                        <Form ref="courseInfo" :model="courseInfo" :rules="ruleValidate">
                            <div class="main-contents">
                                <p>
                                    <FormItem prop="OptName"> <tt id="tt1">*</tt><em>课程名称:</em><i-Input v-model="courseInfo.OptName" :disabled="(courseInfo.Status==2)?true:false" placeholder="请输入课程名称（注:课程名称不要超过50个字符)">
                                    </i-Input></FormItem>
                                </p>
                                <p>                          
                                ...
                                <p> 
                                    ...
                                    <span><FormItem prop="MinAmount">
                                         ...
                                         <tt id="tt2">*</tt><em>最少人数:</em><i-Input v-model="courseInfo.MinAmount" :disabled="(courseInfo.Status==2)?true:false" placeholder="请输入最少招生人数(整数)">
                                    </i-Input></FormItem></span>                               
                                </p>
                                <p>
                                    <span>
                                ...
                                <p>
                                    <!-- <span v-if="courseInfo.Status==2">
                                        <tt id="tt1">*</tt><em>面向年级:</em><i-Input v-model="courseInfo.GradeName" :disabled="true"></i-Input>
                                    </span> -->
                                        ...
                                        <FormItem prop="gradeList"> <tt id="tt1">*</tt><em>面向对象:</em>
                                            <Tree :data="data4" 
                                                ...
                                                ref="Tree"></Tree>
                                            <!-- <i-Button type="primary" @click="gradeModel=true">选择年级/班级</i-Button> -->
                                ...
                                <!-- <Modal v-model="gradeModel" title="选择课程面向的年级/班级" class="Gallery" width="600px">
                                    
                                    <div slot="footer">
                                        <i-Button type="primary" @click="gradeModel=false">保存</i-Button>
                                    </div>
                                </Modal> -->
                                <p>
                                ...
                                <div class="main-contents_img">
                                    ...
                                    <div class="main-contents_imgs">
                                        ...
                                        <Modal v-model="Gallery" title="图片选择" class="Gallery" width="80%">
                                            <p>
                                                <em>当前课程类别:</em>
                                                <i-select class="mr10" style="width:200px;" @on-change="getPictureList"  v-model="selectedCat" transfer>
                        ...
                        <div class="main-contents2 contents_h3" v-if="courseInfo.Status==2">
                            ...
                            <div id="xk1s" v-show="showXS">
                                <div class="Students_buttom">
                                    <!-- @click="Students1 = true" v-show="SX_let" 标记 -->
                                    <i-Button type="primary" icon="android-person" @click="Students1 = true" v-show="true"  >筛选学生</i-Button>
                                    ...
                                    <Modal v-model="modal1" title="提前录取名单" width="80%" class="Modal_box_tq">
                                        ...
                                        <Alert show-icon>
                                            ...
                                            <template slot="desc">
                                                ...
                                                [Sheet1]的第一行为标题行，列名分别为：学生姓名，年级，班级;<br>
                                ...
                                <div class="Students_Form">
                                    ...
                                    <Page show-total :total="XKStuData.total" :page-size="XKStuItem.PageSize" :current="XKStuItem.PageIndex" @on-change="pageUpdate" ></Page>
                                </div>
                            </div>
                        </div>
  ...
  export default {
    ...
    data: function() {
        return {
            ...
            ruleValidate: {
                ...
                gradeList:[{required: true,type:'array', message: '请选择课程面向的对象',trigger: 'change'}],
                ...
                MinAmount:[{ required: false,message: '请输入最少人数(整数)',trigger:'blur'},
            ...
            SYCourseList:[],
            /**图片 */
            courseImportUrl:'/OptCourse/api/base/FileUpload',
            uploadData:{
                AppType:'OptCourse',
            },
            picture:{
    ...
    methods: {
        getGradeClass(data){
            var _this = this;
            _this.classList=[];
            _this.courseInfo.gradeList=[];
            data.forEach(item=>{
                if(!item.children){
                    _this.classList.push(item.ClassID);
                    _this.courseInfo.gradeList.push(item.CodeSerID);
                    //此 可添加年级班级名称，用以显示
                    var bl = false;
                    for(let i=0;i<_this.courseInfo.gradeList.length-1;i++){
                        if(_this.courseInfo.gradeList[i] === item.CodeSerID) bl = true;
                    }
                    if(bl) _this.courseInfo.gradeList.splice(_this.courseInfo.gradeList.length-1,1);
                }
            });
        },
        getTree(){      
        ...
        getActiveListOpen(){   //场次
            ...
            _this.$api.get(APIConfig.OptCourse.GetActiveList,null,(resp)=>{ 
                ...
                _this.getOptCourseList();//标记 原来是关的
            })
        },
        /**课程内容 */
        getActiveListTea(){   //场次
        ...
        getCourseInfo(){    // 课程内容 紧密管理表单验证
            ...
            _this.$api.get(APIConfig.OptCourse.OptCourseDetails,{SerID:_this.currentCourse},(resp)=>{ 
                if(resp.Data != 0){
                    ...
                    if(resp.Data[0].ClassID != null){
                        _this.classList2 = resp.Data[0].ClassID.split(',');
                        _this.classList = _this.classList2;
                    }
                    _this.getTree();
        ...
        pageUpdate(page){
            var _this = this;
            _this.XKStuItem.PageIndex = page;
            _this.getPageOptStuList();
        }
        // validateNum1(){
        //     var _this = this;
